"""This script illustrates the working of the numerical methods in 1D.

Anthony Baston - Oxford - June -2021.

--------------------------------------------------------------------------------

Required variables (adjust as desired):

T        - end time
N        - number of discretised points in space
J        - number of discretised points in time for methods with an implicit component # noqa E501
eps      - the epsilon parameter of the PDE
x_domain - the spatial mesh points, this is fixed to be between 0 and 1.
h        - the spatial mesh size
TOL      - the tolerance for the Newton method in the Implicit and ImexA schemes # noqa E501
max_its  - the maximum number of interations in Newton's methods
dt_ee    - the timestep for the Explicit Euler method - must be O(h^4) for mathematical stability
J_ee     - number of discretised points in time for the explcit method
dt_im    - the timestep for all methods with an implicit component

--------------------------------------------------------------------------------

User inputs:

intial : int
    Type of initial conditions, 0 for smooth and 1 for random
choice : int
    Choice of numerical method, 0 to 5 corresponding to Explicit,
    Implicit, ImexA, ImexB, ImexC and ImexD respectively.

--------------------------------------------------------------------------------

Output:
c_evol: np.array
    Solution of the Cahn-Hilliard equation in 1D, using desired method.


--------------------------------------------------------------------------------
"""
import numpy as np
import matplotlib.pyplot as plt
from numerical_methods import laplacian1D, Explicit, Implicit, \
                              ImexA, ImexB, ImexC, ImexD, initialise
from math import ceil
from get_quantities import get_mass1D, get_energy1D

# Choose method and initial conditions.
choice = 0
initial = 1

# Required variables - adjust as desired.
T = 5*10**-4
N = 100
J_im = 1000
eps = 0.01
x_domain = np.linspace(0, 1, N+1)
h = x_domain[1] - x_domain[0]
TOL = 10**-8
max_its = 1000
dt_ee = h**4
J_ee = ceil(T/dt_ee)
dt_im = T/J_im

# Set up Laplacian operator.
Lp = laplacian1D(N, h)

# Instatiate numerical methods.
methods = [Explicit(dt_ee, eps, Lp), Implicit(dt_im, eps, Lp, TOL, max_its),
           ImexA(dt_im, eps, Lp, TOL, max_its), ImexB(dt_im, eps, Lp),
           ImexC(dt_im, eps, Lp), ImexD(dt_im, eps, Lp)]

# Setup initial conditions - switch=0 for smooth initial conditions,
#                            switch=1 for random initial conditions.
(c0, w0) = initialise(omega_domain=x_domain, laplacian=Lp, epsilson=eps, switch=initial) # noqa E501


# Check choice input.
method = methods[choice]
if choice == 0:
    J = J_ee
    assert dt_ee < 8*h**4, f"For stability, please ensure that the \
                             timestep for the Explicit Euler method \
                             is < 8*h**4 you have entered a value that \
                             is {dt_ee/h**4} times larger that h**4."

elif type(choice) == int and 0 < choice <= 5:
    J = J_im

else:
    raise NotImplementedError(f"Please choose a method by setting choice  \
                                equal to an int from 1 to 6, not {choice} \
                                of type {type(choice)}")

# Solve the equation.
c_evol = np.zeros((N+1, J))
w_evol = np.zeros((N+1, J))
c = c0
w = w0
c_evol[:, 0] = c0
w_evol[:, 0] = w0
for i in range(1, J):
    out = method(c, w)
    c = out[0]
    w = out[1]
    c_evol[:, i] = c

# Plot the evolution of the solution.
fig, ax = plt.subplots(5, 1, sharex=True)
ax[0].plot(x_domain, c_evol[:, 0])
ax[0].set_xlim([0, 1])
ax[0].set_ylim([-1, 1])
for j in range(1, 5):
    ax[j].plot(x_domain, c_evol[:, j*ceil(J/4)-1])
    ax[j].set_xlim([0, 1])
    ax[j].set_ylim([-1, 1])
plt.show()

# Get solution mass evolution over time.
mass_out = get_mass1D(c_evol, h)
plt.figure(6)
plt.plot(mass_out)
plt.show()

# Get solutino energy evolution over time.
energy_out = get_energy1D(c_evol, eps, h)
plt.figure(7)
plt.plot(energy_out)
plt.show()

print("Pause")
